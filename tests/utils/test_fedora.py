...
def test_get_repo_urls():
    ...
    repo_urls = get_repo_urls(
        "f40", "x86_64", exclude_buildroot=False, exclude_debuginfo=False
    )
    assert len(repo_urls) == 3

    # check repo names
    assert "fedora-40-x86_64" in repo_urls
    assert "fedora-debuginfo-40-x86_64" in repo_urls
    assert "fedora-buildroot-40-x86_64" in repo_urls
    ...
    assert "/latest/" not in repo_urls["fedora-buildroot-40-x86_64"]
